...
- `de.json` maps the whitespace-normalized English string to German.
...
- **Plurals built in code**, e.g. `{n} segment{n === 1 ? '' : 's'}` (22 occurrences in 11 files). The English suffix sits in an expression, so a table entry alone yields wrong German ("Abschnitts"). These need small targeted source patches and are not in `de.json` yet.
